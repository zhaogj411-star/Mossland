...
- 默认 RVQ 参数为 `codebook_size=1024`、`num_quantizers=256`。
...
因此 RVQ 默认码率约为当前 FSQ 的 `1.45x`。在当前 `spec_length=32`、`hop=1024`、`sample_rate=44100`、128x 时间压缩口径下，FSQ 约 `2.38 kbps`，RVQ 约 `3.44 kbps`。这是用户确认的质量优先取舍。
...
- `ResidualVectorQuantizer`：按 residual 逐层调用 `VectorQuantizer`，支持 `num_quantizers=256`、`codebook_size=1024`、可选 quantizer dropout。
- `MosslandRVQBottleneck`：Mossland 专用 wrapper，负责 `[B, chunks * 128, 4]` 与 `[B, chunks, 512]` 的 reshape，以及 `codes_to_latents()`。
...
2. RVQ active 阶段：
   - 每个 step 按 `quantizer_dropout_prob` 选择 continuous 或 RVQ latent 给 decoder；
...
`EncoderDecoder.encode(..., discrete=True)` 返回 RVQ codes，形状为：

```text
[chunks, 256]
```

批处理内部可使用 `[B, chunks, 256]`。`decode()` 若收到 integer codes，应调用 `quantizer.codes_to_latents()` 还原为 `[B, chunks * 128, 4]`，再走现有 decoder。

`infer_demo.py` 和 demo callback 继续保存 continuous 与 quantized/RVQ 两条路径，用于比较 RVQ path 与 continuous path 的差异。
...
- 单元测试 RVQ shape：`[B, 128, 4]` 与 `[B, 1, 512]` 往返、multi-chunk 往返、codes decode 往返。
- 单元测试 `EncoderDecoder.encode(discrete=True)` 返回 integer RVQ codes，`decode(codes)` 能进入原 decoder latent shape。
