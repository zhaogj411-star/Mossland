...
- 2026-06-17 进一步修正：用户指出仅把现有 `[128,4]` chunk latent reshape 成 time tokens 仍不能解决时间对齐，因为无时间顺序的根因在 encoder Transformer 的 learned latent query / global summarization。真正时间对齐需要改变 encoder 输出方式：输出 token 必须由固定时间位置锚定，且最好使用局部/窗口注意力或卷积式 token encoder，不能只是对现有 summary latent 重新排列。
