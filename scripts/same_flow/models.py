...
class ZeroTokenProjection(nn.Module):
    """Zero-initialized per-token projection for consistency residual output."""

    def __init__(self, in_channels: int, out_channels: int, output_scale: float = 1.0):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return tokens.transpose(1, 2) * self.output_scale


class ZeroConv2dProjection(nn.Module):
    """Zero-initialized 2D projection matching Music2Latent residual head semantics."""
...
class SameFlow2DLegacy(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.activation_out = nn.SiLU()
        self.output_proj = ZeroConv2dProjection(
            input_channels,
            self.data_channels,
            kernel_size=3,
            output_scale=self.output_residual_scale,
        )
...
class SpectralFrameAdapter(nn.Module):
    ...
    def __init__(
        ...
        patch_dim = self.data_channels * self.freq_patch_size
...
class SameFlowConsistencyAutoencoder(nn.Module):
    """SAME frame-token consistency autoencoder with full-frame residual output."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.output_proj = ZeroTokenProjection(
            input_channels,
            self.data_channels * self.freq_bins,
            output_scale=self.output_residual_scale,
        )
        self.max_waveform_length_encode = int(max_waveform_length_encode)
        self.max_batch_size_encode = int(max_batch_size_encode)
