...
def _si_sdr_db(source, pred):
    source = source.reshape(-1).float()
    pred = pred.reshape(-1).float()
    source = source - source.mean()
    pred = pred - pred.mean()
    target = (pred @ source) * source / source.pow(2).sum().clamp_min(1e-12)
    noise = pred - target
    ratio = target.pow(2).sum() / noise.pow(2).sum().clamp_min(1e-12)
    return float(10.0 * torch.log10(ratio.clamp_min(1e-12)))
...
def _mrstft_metrics(source, pred):
    ...
    for n_fft, hop_length in scales:
        ...
        spectral_convergence.append(
            (source_spec - pred_spec).norm() / source_spec.norm().clamp_min(1e-12)
        )
        log_mag_l1.append(
            (
    ...
    return {
        "mrstft_sc": float(torch.stack(spectral_convergence).mean()),
        "mrstft_log_mag_l1": float(torch.stack(log_mag_l1).mean()),
    }


def _logmel_metrics(source, pred, sample_rate, n_fft=2048, hop_length=512):
    source = source.mean(dim=0).float()
    pred = pred.mean(dim=0).float()
    window = torch.hann_window(n_fft, device=source.device)
    ...
    def logmel(audio):
        ...
        mel = torch.matmul(mel_fb.T, spec).clamp_min(1e-5)
        return torch.log(mel)

    source_mel = logmel(source)
    pred_mel = logmel(pred)
...
def _prediction_metrics(source, pred, sample_rate, prefix=""):
    metrics = {
        f"{prefix}source_rms": _rms(source),
        f"{prefix}prediction_rms": _rms(pred),
        f"{prefix}corr": _corrcoef(source, pred),
        f"{prefix}snr_db": _snr_db(source, pred),
        f"{prefix}si_sdr_db": _si_sdr_db(source, pred),
        f"{prefix}lsd_db": _lsd_db(source, pred, sample_rate),
    }
    metrics.update(
        {
            f"{prefix}{key}": value
            for key, value in _band_metrics(source, pred, sample_rate).items()
        }
    )
    metrics.update(
        {
            f"{prefix}{key}": value
            for key, value in _mrstft_metrics(source, pred).items()
        }
    )
    metrics.update(
        {
            f"{prefix}{key}": value
            for key, value in _logmel_metrics(source, pred, sample_rate).items()
        }
    )
    gain = _optimal_gain(source, pred)
    metrics.update(
        {
            f"{prefix}optimal_gain": gain,
            f"{prefix}optimal_gain_snr_db": _snr_db(source, pred * gain),
            f"{prefix}optimal_gain_prediction_rms": _rms(pred * gain),
        }
    )
    return metrics
...
def main():
    ...
    parser.add_argument(
        "--eval-rvq",
        action="store_true",
        help="Also decode quantized latent for quantizer-enabled checkpoints.",
    )
    ...
    for index in range(min(args.max_items, len(dataset))):
        ...
        row.update(_prediction_metrics(source_aligned, pred_aligned, sample_rate))
        if args.eval_rvq:
            if not model.has_quantizer:
                raise RuntimeError("--eval-rvq requires a quantizer-enabled model")
            quantized = model.quantize_representation(
                representation,
                detach_encoder=True,
                n_quantizers=args.n_quantizers,
            )
            rvq_pred = _as_audio_channels(
                model.decode(
                    quantized.discrete,
                    denoising_steps=args.denoising_steps,
                    target_length=source.shape[-1],
            ...
            rvq_source, rvq_pred = _align_audio(source, rvq_pred)
            rvq_suffix = (
                f"rvq{args.n_quantizers}"
                if args.n_quantizers is not None
                else "rvq"
            )
            rvq_path = item_dir / f"reconstruct_{rvq_suffix}_{args.variant}.wav"
            _save_audio(rvq_path, rvq_pred, sample_rate)
            row.update(
                {
                    "rvq_prediction_wav": str(rvq_path),
                    "rvq_n_quantizers": args.n_quantizers,
                    "rvq_latent_shape": list(quantized.discrete.shape),
                    "rvq_latent_mean": float(
                        quantized.discrete.detach().float().mean()
                    ),
                    "rvq_latent_std": float(
                        quantized.discrete.detach().float().std()
                    ),
                    "rvq_commitment_loss": float(
                        quantized.commitment_loss.detach().float()
                    ),
                    "rvq_hidden_recon_loss": float(
                        quantized.distill_loss.detach().float()
                    ),
                }
            )
            row.update(_prediction_metrics(rvq_source, rvq_pred, sample_rate, "rvq_"))
        if args.direct_sigma is not None:
            ...
            row.update(_prediction_metrics(direct_source, direct_pred, sample_rate, "direct_"))
    ...
    summary.update(
        {
            ...
            "nonquiet_snr_db_mean": _mean([row["snr_db"] for row in nonquiet_rows]),
            "nonquiet_si_sdr_db_mean": _mean(
                [row["si_sdr_db"] for row in nonquiet_rows]
            ),
            ...
            "quiet_source_rms_mean": _mean([row["source_rms"] for row in quiet_rows]),
        ...
        summary.update(
            {
                ...
                "direct_si_sdr_db_mean": _mean(
                    [row["direct_si_sdr_db"] for row in rows]
                ),
                ...
                "direct_rms_mean": _mean(
                    [row["direct_prediction_rms"] for row in rows]
                ),
                ...
                "nonquiet_direct_si_sdr_db_mean": _mean(
                    [row["direct_si_sdr_db"] for row in nonquiet_rows]
                ),
                "nonquiet_direct_mrstft_sc_mean": _mean(
                    [row["direct_mrstft_sc"] for row in nonquiet_rows]
                ),
                "nonquiet_direct_logmel_l1_mean": _mean(
                    [row["direct_logmel_l1"] for row in nonquiet_rows]
                ),
                "quiet_direct_rms_mean": _mean(
                    [row["direct_prediction_rms"] for row in quiet_rows]
                ),
            }
        )
        if args.mismatch_latent_offset:
            summary.update(
                {
                    ...
                    "mismatch_direct_rms_mean": _mean(
                        [row["mismatch_direct_prediction_rms"] for row in rows]
                    ),
    ...
    if args.eval_rvq:
        summary.update(
            {
                "rvq_corr_mean": _mean([row["rvq_corr"] for row in rows]),
                "rvq_snr_db_mean": _mean([row["rvq_snr_db"] for row in rows]),
                "rvq_si_sdr_db_mean": _mean(
                    [row["rvq_si_sdr_db"] for row in rows]
                ),
                "rvq_lsd_db_mean": _mean([row["rvq_lsd_db"] for row in rows]),
                "rvq_mrstft_sc_mean": _mean(
                    [row["rvq_mrstft_sc"] for row in rows]
                ),
                "rvq_logmel_l1_mean": _mean(
                    [row["rvq_logmel_l1"] for row in rows]
                ),
                "rvq_latent_std_mean": _mean(
                    [row["rvq_latent_std"] for row in rows]
                ),
                "rvq_prediction_rms_mean": _mean(
                    [row["rvq_prediction_rms"] for row in rows]
                ),
                "rvq_hf_rms_ratio_mean": _mean(
                    [row["rvq_hf_rms_ratio"] for row in rows]
                ),
                "rvq_lf_rms_ratio_mean": _mean(
                    [row["rvq_lf_rms_ratio"] for row in rows]
                ),
                "rvq_hf_lsd_db_mean": _mean(
                    [row["rvq_hf_lsd_db"] for row in rows]
                ),
                "rvq_lf_lsd_db_mean": _mean(
                    [row["rvq_lf_lsd_db"] for row in rows]
                ),
                "rvq_commitment_loss_mean": _mean(
                    [row["rvq_commitment_loss"] for row in rows]
                ),
                "rvq_hidden_recon_loss_mean": _mean(
                    [row["rvq_hidden_recon_loss"] for row in rows]
                ),
                "nonquiet_rvq_corr_mean": _mean(
                    [row["rvq_corr"] for row in nonquiet_rows]
                ),
                "nonquiet_rvq_snr_db_mean": _mean(
                    [row["rvq_snr_db"] for row in nonquiet_rows]
                ),
                "nonquiet_rvq_si_sdr_db_mean": _mean(
                    [row["rvq_si_sdr_db"] for row in nonquiet_rows]
                ),
                "nonquiet_rvq_mrstft_sc_mean": _mean(
                    [row["rvq_mrstft_sc"] for row in nonquiet_rows]
                ),
                "nonquiet_rvq_logmel_l1_mean": _mean(
                    [row["rvq_logmel_l1"] for row in nonquiet_rows]
                ),
                "quiet_rvq_prediction_rms_mean": _mean(
                    [row["rvq_prediction_rms"] for row in quiet_rows]
                ),
            }
        )
