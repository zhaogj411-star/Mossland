...
from .models import Music2LatentSameFlow
...
class SameFlowTrainingWrapper(CodecTrainingBase):
    def __init__(
        ...
        training_objective: str = "consistency",
        max_denoise_sigma: float | None = None,
        clean_loss_type: str = "pseudo_huber",
        denoising_anchor_weight: float = 0.0,
        high_freq_anchor_weight: float = 0.0,
        high_freq_anchor_start_bin: int = 320,
        ...
        self.max_denoise_sigma = max_denoise_sigma
        self.clean_loss_type = str(clean_loss_type)
        self.denoising_anchor_weight = float(denoising_anchor_weight)
        self.high_freq_anchor_weight = float(high_freq_anchor_weight)
        self.high_freq_anchor_start_bin = int(high_freq_anchor_start_bin)
        self.latent_prior_loss_weight = float(latent_prior_loss_weight)
        if use_ema:
            self.ema = EMA(
                self.model,
                beta=0.9999,
    ...
    def _clean_anchor_loss(self, predicted, representation):
        if self.clean_loss_type == "mse":
            return F.mse_loss(predicted.float(), representation.float())
        if self.clean_loss_type != "pseudo_huber":
            raise ValueError(f"Unsupported clean_loss_type={self.clean_loss_type!r}")
    ...
    def _high_freq_anchor_loss(self, predicted, representation):
        start_bin = min(
            max(self.high_freq_anchor_start_bin, 0),
            representation.shape[-2] - 1,
        )
        if self.clean_loss_type == "mse":
            return F.mse_loss(
                predicted[..., start_bin:, :].float(),
                representation[..., start_bin:, :].float(),
            )
        return pseudo_huber_loss(
            predicted[..., start_bin:, :].float(),
            representation[..., start_bin:, :].float(),
    ...
    def _max_denoise_loss(self, representation, latent_override):
        batch_size = representation.shape[0]
        sigma_value = (
            self.model.sigma_max
            if self.max_denoise_sigma is None
            else float(self.max_denoise_sigma)
        )
        sigma = torch.full(
            (batch_size,),
            float(sigma_value),
        ...
        noisy = add_noise(representation, noise, sigma)
        ...
        clean_loss = self._clean_anchor_loss(predicted, representation)
        high_freq_anchor_loss = torch.zeros((), device=representation.device)
        loss = clean_loss
        if self.high_freq_anchor_weight > 0:
            high_freq_anchor_loss = self._high_freq_anchor_loss(
                predicted,
                representation,
            )
            loss = loss + self.high_freq_anchor_weight * high_freq_anchor_loss
        metrics = {
            "loss/consistency": loss.detach(),
            "loss/consistency_pair": torch.zeros((), device=representation.device),
            "loss/denoising_anchor": clean_loss.detach(),
            "loss/high_freq_anchor": high_freq_anchor_loss.detach(),
            "sigma/weight_mean": torch.ones((), device=representation.device),
            "sigma/step": torch.ones((), device=representation.device),
            "sigma/low_mean": torch.zeros((), device=representation.device),
            "sigma/high_mean": sigma.mean().detach(),
        }
        return loss, metrics, predicted, representation.detach()

    def fixed_max_denoise_step(self, representation, latent_override, noisy, sigma):
        predicted = self.model(
            representation,
            noisy,
            sigma=sigma,
        ...
        metrics = {
            ...
            "sigma/step": torch.ones((), device=representation.device),
            "sigma/low_mean": torch.zeros((), device=representation.device),
            "sigma/high_mean": sigma.detach().float().mean(),
        }
    ...
    def _consistency_loss(self, representation, latent_override):
        ...
        loss = official_huber_loss(
            predicted,
            target,
            w=(1.0 / sigma_delta),
            c=self.consistency_loss_delta,
        )
        anchor_loss = self._clean_anchor_loss(predicted, representation)
        if self.denoising_anchor_weight > 0:
            loss = loss + self.denoising_anchor_weight * anchor_loss
        high_freq_anchor_loss = torch.zeros((), device=representation.device)
        if self.high_freq_anchor_weight > 0:
            high_freq_anchor_loss = self._high_freq_anchor_loss(
                predicted,
        ...
        metrics = {
            ...
            "loss/denoising_anchor": anchor_loss.detach(),
            "loss/high_freq_anchor": high_freq_anchor_loss.detach(),
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.training_objective == "consistency":
            consistency_loss, metrics, predicted, target = self._consistency_loss(
                representation,
                latent_override=latent_override,
            )
        elif self.training_objective == "max_denoise":
            consistency_loss, metrics, predicted, target = self._max_denoise_loss(
                representation,
                latent_override=latent_override,
            )
        else:
        ...
        if self.latent_prior_loss_weight > 0 and hasattr(self.model, "latent_prior"):
            prior = self.model.latent_prior(source_latent)
            self._assert_finite("latent_prior", prior, info)
            prior_loss = F.mse_loss(
                prior.float(),
                representation.float(),
            )
            loss = loss + self.latent_prior_loss_weight * prior_loss
            self.log("loss/latent_prior", prior_loss.detach(), prog_bar=False)
