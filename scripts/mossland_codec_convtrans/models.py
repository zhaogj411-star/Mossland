...
class ConvEncoderDownsampleConv(nn.Module):
...
class ConvEncoderDownsampleFreqConv(nn.Module):
    def __init__(self, in_channels, out_channels=None, normalize=False):
        super().__init__()
        self.normalize = bool(normalize)
        if out_channels is None:
...
class ConvEncoderMultiheadAttention(nn.MultiheadAttention):
    def _reset_parameters(self):
        super()._reset_parameters()
        self.out_proj = zero_init(self.out_proj)


class ConvEncoderAttention(nn.Module):
    def __init__(self, dim, heads=4, normalize=True, use_2d=False):
        super().__init__()
        self.normalize = bool(normalize)
        self.use_2d = bool(use_2d)
        self.mha = ConvEncoderMultiheadAttention(
            embed_dim=dim,
            num_heads=heads,
            dropout=0.0,
            add_zero_attn=False,
    ...
    def forward(self, x):
        ...
        if self.use_2d:
            x = x.permute(0, 3, 2, 1)
            batch, length, freq, channels = x.shape
            x = x.reshape(batch * length, freq, channels)
        else:
            x = x.permute(0, 2, 1)
        x = self.mha(x, x, x, need_weights=False)[0]
        if self.use_2d:
            x = x.reshape(batch, length, freq, channels).permute(0, 3, 2, 1)
        else:
            x = x.permute(0, 2, 1)
        return x + residual


class ConvEncoderResBlock(nn.Module):
    def __init__(
        self,
        in_channels,
        out_channels,
        ...
        conv = nn.Conv2d if self.use_2d else nn.Conv1d
        self.conv1 = conv(
            in_channels,
            out_channels,
...
class ConvEncoder(nn.Module):
    """Local conv encoder with checkpoint-compatible latent projection names."""

    def __init__(
        self,
        base_channels=64,
        layers_list_encoder=None,
        multipliers_list=None,
        attention_list_encoder=None,
        freq_downsample_list=None,
        ...
        min_res_dropout=16,
        dropout_rate=0.0,
        latent_head_init_std=0.0,
        latent_tanh_scale=1.0,
    ):
        super().__init__()
        if layers_list_encoder is None:
            layers_list_encoder = [1, 1, 1, 1, 1]
        if multipliers_list is None:
            multipliers_list = [1, 2, 4, 4, 4]
        if attention_list_encoder is None:
            attention_list_encoder = [0, 0, 1, 1, 1]
        if freq_downsample_list is None:
        ...
        self.latent_head_init_std = float(latent_head_init_std)
        self.latent_tanh_scale = float(latent_tanh_scale)
        if self.latent_head_init_std < 0:
            raise ValueError(
                f"latent_head_init_std must be non-negative, got {self.latent_head_init_std}"
            )
        if self.latent_tanh_scale <= 0:
            raise ValueError(
                f"latent_tanh_scale must be positive, got {self.latent_tanh_scale}"
            )
        ...
        for i, (num_layers, multiplier) in enumerate(
            zip(self.layers_list, multipliers_list, strict=True)
        ):
            ...
            for _ in range(num_layers):
                down_layers.append(
                    ConvEncoderResBlock(
                        ...
                        attention=attention_list_encoder[i] == 1,
                        heads=heads,
                        use_2d=True,
        ...
        self.norm_out = nn.LayerNorm(bottleneck_base_channels)
        self.activation_out = nn.Identity()
        self.conv_out = nn.Linear(bottleneck_base_channels, bottleneck_channels)
        if self.latent_head_init_std == 0:
    ...
    def project_features(self, x):
        for layer in self.bottleneck_layers:
            x = layer(x)
        hidden = x
        continuous, pre_tanh = self.hidden_to_latent(hidden)
        return continuous, hidden, pre_tanh

    def hidden_to_latent(self, hidden):
        x = hidden.transpose(1, 2)
        x = self.norm_out(x)
        x = self.activation_out(x)
        pre_tanh = self.conv_out(x)
        pre_tanh = pre_tanh * self.latent_tanh_scale
        continuous = self.activation_bottleneck(pre_tanh)
        return (
            continuous.transpose(1, 2).contiguous(),
            pre_tanh.transpose(1, 2).contiguous(),
        )
    ...
    def forward(self, x, extract_features=False, return_hidden=False):
        ...
        if extract_features:
            return x
        continuous, hidden, _pre_tanh = self.project_features(x)
        if return_hidden:
...
class MosslandCodecConvTrans(nn.Module):
    ...
    def __init__(
        ...
        conv_base_channels: int = 128,
        conv_layers_list_encoder: list[int] | None = None,
        conv_multipliers_list: list[int] | None = None,
        conv_attention_list_encoder: list[int] | None = None,
        conv_freq_downsample_list: list[int] | None = None,
        conv_bottleneck_base_channels: int = 1024,
        conv_num_bottleneck_layers: int = 9,
        conv_hop: int | None = None,
        conv_frequency_scaling: bool = True,
        conv_heads: int = 8,
        conv_normalization: bool = True,
        conv_pre_normalize_2d_to_1d: bool = True,
        conv_pre_normalize_downsampling_encoder: bool = True,
        conv_dropout_rate: float = 0.0,
        conv_min_res_dropout: int = 16,
        conv_latent_head_init_std: float = 0.0,
        conv_latent_tanh_scale: float = 1.0,
    ):
        super().__init__()
        ...
        if conv_layers_list_encoder is None:
            conv_layers_list_encoder = [2, 3, 4, 4, 3]
        if conv_multipliers_list is None:
            conv_multipliers_list = [1, 2, 4, 4, 4]
        if conv_attention_list_encoder is None:
            conv_attention_list_encoder = [0, 0, 1, 1, 1]
        if conv_freq_downsample_list is None:
            conv_freq_downsample_list = [1, 0, 0, 0]
        if conv_hop is None:
            conv_hop = self.hop * self.fac // 4 if hasattr(self, "hop") else hop * fac // 4
        ...
        self.conv_base_channels = int(conv_base_channels)
        self.conv_layers_list_encoder = list(conv_layers_list_encoder)
        self.conv_multipliers_list = list(conv_multipliers_list)
        self.conv_attention_list_encoder = list(conv_attention_list_encoder)
        self.conv_freq_downsample_list = list(conv_freq_downsample_list)
        self.conv_bottleneck_base_channels = int(conv_bottleneck_base_channels)
        self.conv_num_bottleneck_layers = int(conv_num_bottleneck_layers)
        self.conv_hop = int(conv_hop)
        self.conv_frequency_scaling = bool(conv_frequency_scaling)
        self.conv_heads = int(conv_heads)
        self.conv_normalization = bool(conv_normalization)
        self.conv_pre_normalize_2d_to_1d = bool(conv_pre_normalize_2d_to_1d)
        self.conv_pre_normalize_downsampling_encoder = bool(conv_pre_normalize_downsampling_encoder)
        self.conv_dropout_rate = float(conv_dropout_rate)
        self.conv_min_res_dropout = int(conv_min_res_dropout)
        self.conv_latent_head_init_std = float(conv_latent_head_init_std)
        self.conv_latent_tanh_scale = float(conv_latent_tanh_scale)
        self.conv_time_downsample_ratio = 2 ** sum(1 for item in self.conv_freq_downsample_list if item == 0)
        if self.spec_length % self.conv_time_downsample_ratio != 0:
            raise ValueError(
                f"spec_length={self.spec_length} must be divisible by conv_time_downsample_ratio="
                f"{self.conv_time_downsample_ratio}"
            )
        self.conv_steps_per_chunk = self.spec_length // self.conv_time_downsample_ratio
        if self.conv_steps_per_chunk != self.rvq_tokens_per_chunk:
            raise ValueError(
                "conv bottleneck requires one conv step per RVQ token: "
                f"conv_steps_per_chunk={self.conv_steps_per_chunk}, "
                f"rvq_tokens_per_chunk={self.rvq_tokens_per_chunk}"
            )
        expected_freq = self.hop * (self.fac // 2)
        if self.conv_hop * 2 != expected_freq:
            raise ValueError(
                f"conv_hop*2 must match representation freq bins ({expected_freq}), got conv_hop={self.conv_hop}"
            )
        ...
        self.conv_encoder = ConvEncoder(
            base_channels=self.conv_base_channels,
            layers_list_encoder=self.conv_layers_list_encoder,
            multipliers_list=self.conv_multipliers_list,
            attention_list_encoder=self.conv_attention_list_encoder,
            freq_downsample_list=self.conv_freq_downsample_list,
            bottleneck_base_channels=self.conv_bottleneck_base_channels,
            num_bottleneck_layers=self.conv_num_bottleneck_layers,
            frequency_scaling=self.conv_frequency_scaling,
            heads=self.conv_heads,
            normalization=self.conv_normalization,
            pre_normalize_2d_to_1d=self.conv_pre_normalize_2d_to_1d,
            pre_normalize_downsampling_encoder=self.conv_pre_normalize_downsampling_encoder,
            hop=self.conv_hop,
            data_channels=self.stft_channels,
            bottleneck_channels=self.rvq_token_dim,
            min_res_dropout=self.conv_min_res_dropout,
            dropout_rate=self.conv_dropout_rate,
            latent_head_init_std=self.conv_latent_head_init_std,
            latent_tanh_scale=self.conv_latent_tanh_scale,
        )
        self._last_conv_pre_tanh = None
        self._last_conv_tokens = None
    ...
    def pack_rvq_tokens(self, latents: torch.Tensor) -> torch.Tensor:
        """Convert decoder-expanded latents [B, chunks*128, 4] to RVQ tokens.

        This is kept for compatibility with older call sites. The clean data path
        is conv encoder -> [B, 64, chunks*8] -> RVQ, and only expands to
        [B, chunks*128, 4] right before the Transformer decoder.
        """
        return self.decoder_latents_to_rvq_tokens(latents)

    def unpack_rvq_tokens(self, packed: torch.Tensor) -> torch.Tensor:
        """Convert RVQ tokens [B, 64, chunks*8] to decoder latents."""
        return self.rvq_tokens_to_decoder_latents(packed)

    def decoder_latents_to_rvq_tokens(self, latents: torch.Tensor) -> torch.Tensor:
        """Pack [B, chunks*num_latents, 4] into [B, 64, chunks*8]."""
    ...
    def rvq_tokens_to_decoder_latents(self, packed: torch.Tensor) -> torch.Tensor:
        """Expand [B, 64, chunks*8] to [B, chunks*128, 4] for the decoder."""
    ...
    def encode_bottleneck(
        ...
        continuous_packed = self.encoder_forward(
            x,
            dont_quantize=True,
            log_magnitude=log_magnitude,
        )
        continuous = self.rvq_tokens_to_decoder_latents(continuous_packed)
        if self.rvq is not None:
            if not quantize:
                return BottleneckOutput(
                    latents=continuous,
                    continuous=continuous,
                    packed_continuous=continuous_packed,
                    quantized=False,
                    rvq_tokens_per_chunk=self.rvq_tokens_per_chunk,
                )
            quantizer_input = continuous_packed.detach() if detach_encoder else continuous_packed
            (
                packed_quantized,
                codes,
                _rvq_aux_loss,
            ) = self.rvq(quantizer_input, n_quantizers=n_quantizers)
            quantized = self.rvq_tokens_to_decoder_latents(packed_quantized)
            quantization_error = F.mse_loss(
                packed_quantized.float(),
                continuous_packed.detach().float(),
            )
    ...
    def encode_packed_from_representation(
        ...
        if packed is None:
            packed = self.decoder_latents_to_rvq_tokens(encoded.latents)
        return packed
    ...
    def encode_codes_from_representation(
        ...
        if self.rvq is not None:
            continuous_packed = self.encoder_forward(x, dont_quantize=True)
            return self.rvq.encode_codes(continuous_packed, n_quantizers=n_quantizers)
        encoded = self.encode_bottleneck(
            x,
            quantize=True,
            n_quantizers=n_quantizers,
    ...
    def _validate_rvq_tokens(self, tokens: torch.Tensor) -> torch.Tensor:
        """Validate conv/RVQ tokens with shape [B, 64, chunks*8]."""
        if tokens.ndim != 3:
            raise ValueError(f"RVQ tokens must have shape [B,C,T], got {tuple(tokens.shape)}")
        _batch, channels, steps = tokens.shape
        if channels != self.rvq_token_dim:
            raise ValueError(f"expected rvq_token_dim={self.rvq_token_dim}, got {channels}")
        if steps % self.rvq_tokens_per_chunk != 0:
            raise ValueError(
                f"RVQ token steps {steps} must be divisible by "
                f"rvq_tokens_per_chunk={self.rvq_tokens_per_chunk}"
            )
        return tokens

    def encoder_forward(self, x, dont_quantize=False, log_magnitude=False):
        """Encode STFT chunks to continuous RVQ tokens [B, 64, chunks*8].

        Args:
            x: Input spectrogram chunks [B, C, F, T], T multiple of spec_length.
            dont_quantize: Kept for API compatibility; quantization is handled by encode_bottleneck().
            log_magnitude: Kept for API compatibility.
        Returns:
            Tensor of continuous RVQ tokens with shape [B, rvq_token_dim, chunks*rvq_tokens_per_chunk].
        """
        if x.shape[-1] % self.spec_length != 0:
            raise ValueError(f"Input shape {x.shape[-1]} is not divisible by spec_length={self.spec_length}.")
        conv_tokens, _hidden, pre_tanh = self.conv_encoder.project_features(
            self.conv_encoder.encode_features(x)
        )
        self._last_conv_pre_tanh = pre_tanh
        self._last_conv_tokens = conv_tokens
        return self._validate_rvq_tokens(conv_tokens)

    def encoder_forward_fast(self, x, dont_quantize=False, log_magnitude=False):
        """Compatibility alias for inference code paths that previously used torch.compile."""
        return self.encoder_forward(x, dont_quantize=dont_quantize, log_magnitude=log_magnitude)
...
# Backward-compatible alias for utilities that still import the old class name.
MosslandCodecTransformer = MosslandCodecConvTrans
