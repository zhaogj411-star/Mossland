...
def get_sigma_continuous(i, sigma_min=0.002, sigma_max=80.0, rho=7.0):
...
def get_sigma_step_continuous(sigma_i, step, sigma_min=0.002, sigma_max=80.0, rho=7.0):
...
def reverse_step(x, noise, sigma, sigma_min=0.002):
...
@torch.no_grad()
def distribute(model, x, max_batch_size, device, *args, mixed_precision=True, **kwargs):
...
def download_model():
    """检查默认 Mossland codec checkpoint 是否存在。"""
    filepath = os.path.abspath(__file__)
    lib_root = os.path.dirname(filepath)
    local_dir = os.path.join(lib_root, "models")
    local_path = os.path.join(local_dir, "mossland-codec.pt")

    if os.path.exists(local_path):
        return

    raise RuntimeError(
        "未找到默认 Mossland codec checkpoint。请传入 load_path_inference，"
        f"或把权重放到 {local_path}。"
    )
