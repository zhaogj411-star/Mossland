...
def normalize_complex_realimag(x, alpha_rescale=0.65, beta_rescale=0.34):
    ...
    This is equivalent to beta * abs(c) ** alpha * exp(1j * angle(c)), but
    avoids complex division/multiplication so it works on devices with limited
...
def to_representation_encoder(x, hop, fac, alpha_rescale=0.65, beta_rescale=0.34):
    return wv2realimag(
        x,
        hop,
        fac,
...
def to_representation(x, hop, fac, alpha_rescale=0.65, beta_rescale=0.34):
    return wv2realimag(
        x,
        hop,
        fac,
...
def to_waveform(x, hop, fac, alpha_rescale=0.65, beta_rescale=0.34):
    return realimag2wv(
        x,
        hop,
        fac,
...
def wv2mel(x, sample_rate=48000):
    melmat = create_melmat(sample_rate=sample_rate)
    melmat = melmat.to(x.device)
    return torch.tensordot(wv2spec(x), melmat, dims=([-2],[0])).permute(0,2,1)

def plot_audio(wv, sample_rate=48000):
    import matplotlib.pyplot as plt
    spec = wv2mel(wv, sample_rate=sample_rate)
...
def plot_audio_compare(wv1,wv2, sample_rate=48000):
    import matplotlib.pyplot as plt
    spec1 = []
    spec2 = []
    for w1,w2 in zip(wv1,wv2):
        spec1.append(wv2mel(w1.unsqueeze(0), sample_rate=sample_rate).squeeze(0)[..., :1024])
        spec2.append(wv2mel(w2.unsqueeze(0), sample_rate=sample_rate).squeeze(0)[..., :1024])
