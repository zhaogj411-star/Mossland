...
def block_causal_attention_mask(block_size: int) -> tuple[str, int]:
    return ("block_causal", int(block_size))


def is_block_causal_attention_mask(attn_mask) -> bool:
    return (
        isinstance(attn_mask, tuple)
        and len(attn_mask) == 2
        and attn_mask[0] == "block_causal"
    )


def flash_attention_available(q: torch.Tensor) -> bool:
...
def scaled_dot_product_attention(q, k, v, *, is_causal=False, scale=None, attn_mask=None):
    if exists(attn_mask):
    ...
    if flash_attention_available(q):
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, num_groups=None, attn_mask=None):
        ...
        if is_block_causal_attention_mask(attn_mask):
            block_size = int(attn_mask[1])
            if n != 2 * block_size:
                raise ValueError(
                    f"block_causal attention mask expected sequence length "
                    f"{2 * block_size}, got {n}"
                )
            out_left = scaled_dot_product_attention(
                q[:, :, :block_size],
                k[:, :, :block_size],
                v[:, :, :block_size],
                scale=scale_factor,
            )
            out_right = scaled_dot_product_attention(
                q[:, :, block_size:],
                k,
                v,
                scale=scale_factor,
            ...
            out = scaled_dot_product_attention(
                ...
                k,
                v,
                is_causal=self.causal if not exists(attn_mask) else False,
                scale=scale_factor,
                attn_mask=attn_mask,
            )
